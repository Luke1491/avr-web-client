...
static uint8_t gwmacaddr[6] = {0xb8, 0x88, 0xe3, 0x94, 0xce, 0x88};   //  {0x1C, 0x39, 0x47, 0xCF, 0xDF, 0xCB};<--mac lenowo dla urzêdu   {0x64, 0x31, 0x50, 0x8d, 0xf3, 0x68};//<-- mac laptopa z navdeckiem;  mac karty dell:{0x78, 0x2b, 0xcb, 0xd8, 0x3d, 0xe5}; mac karty dell inspiron: {0x64, 0x00, 0x6a, 0xfa, 0x86, 0x24}; mac lenovo Kamila: {0xb8, 0x88, 0xe3, 0x94, 0xce, 0x88};
...
void send_udp_prepare(uint8_t *buf,uint16_t sport, uint8_t *dip, uint16_t dport)
{
        ...
        // total length field in the IP header must be set:
        buf[IP_TOTLEN_H_P]=0;
		//buf[0x12]= 128; //-->identification parameter 
